...
import BitcoinWallet from "@/lib/derivation/bitcoin";
  ...
  async function generateWallet() {
    try {
      ...
      if (cryptoName === "Solana") {
        ...
        keyPair = BitcoinWallet(mnemonicsString);
      }
          ...
          <div className="flex gap-3">
            {CRYPTOS.map((crypto) => (
              <div
                ...
                className={`px-3 py-1.5 rounded-md cursor-pointer text-xs md:text-sm flex items-center gap-2 border border-white/40  ${
                  cryptoName == crypto.name ? "bg-white text-black" : ""
                }`}
                key={crypto.id}
              >
                <img
